version = 99.0.0
provider_macos_path = registry.terraform.io/ketion-so/notion/$(version)/darwin_amd64/
...
install_macos:
	@go build -o terraform-provider-notion_$(version)
	@mkdir -p ~/Library/Application\ Support/io.terraform/plugins/$(provider_macos_path)
	@mv terraform-provider-notion_$(version)  ~/Library/Application\ Support/io.terraform/plugins/$(provider_macos_path)
